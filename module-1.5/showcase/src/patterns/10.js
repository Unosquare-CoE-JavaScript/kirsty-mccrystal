...
const useClapAnimation = ({ clapEl, countEl, clapTotalEl }) => {
  const [animationTimeline, setAnimationTimeline] = useState(
    () => new mojs.Timeline()
  ); //Passing a function reference to avoid invoke all the time
  ...
  }, [clapEl, countEl, clapTotalEl]); //Empty array invoked once
...
// Custom hook for getting prev prop/state
...
const useClapState = (initialState = INITIAL_STATE, reducer = internalReducer) => {
  const userIntitialState = useRef(initialState)
  ...
  const reset = useCallback(() => {
    if(prevCount !== count) {
      dispatch({type: 'reset', payload: userIntitialState.current})
      resetRef.current++
    }
  }, [prevCount, count])
...
const useEffectAfterMount = (cb, deps) => {
  ...
  useEffect(() => {
    if (!componentJustMounted.current) {
      return cb(); // return helps to clean up
    }
...
const Usage = () => {
  ...
  const {
    ...
    reset,
    resetDep
  } = useClapState(userIntitialState, reducer);
  ...
  return (
    ...
    <section>
      ...
      <pre style={{color: 'red'}}>{isClappedTooMuch ? `You have clapped too much.` : ''}</pre>
    </section>
    </div>
  );
};
